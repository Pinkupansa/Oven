...
EditorCamera::EditorCamera(float fov, float aspectRatio, float nearClip, float farClip, EditorContext* context)
    : m_FOV(fov), m_AspectRatio(aspectRatio), m_NearClip(nearClip), m_FarClip(farClip),
      Camera(glm::perspective(glm::radians(fov), aspectRatio, nearClip, farClip)), m_Context(context)
{ UpdateView(); }
...
void EditorCamera::OnUpdate()
{
    ...
    {
        if (Input::MouseButtonPressed(OvenMouseButton::Right))
        {
            if (Input::KeyPressed(OvenKey::LeftShift))
            {
                MouseZoom(delta.y);
            }
            else
...
void EditorCamera::OnEvent(Event& e)
{
    ...
    dispatcher.Dispatch<MouseScrolledEvent>(OVEN_BIND_EVENT_FN(EditorCamera::OnMouseScroll));
}
